...
public class Clientview extends PaginatedTableView<Client,Clientview,Clientview> {
    ...
    private List<ClientGender> genders = new ArrayList<>();
    private ClientGender selectedGender;

    @PostConstruct
    public void init(){
        clientservice= ApplicationContextProvider.getBean(ClientService.class);
        this.genders = Arrays.asList(ClientGender.values());
...
//this.searchFields = Arrays.asList(new Search())
        this.totalclients = super.getTotalRecords();
        this.numberMale = clientservice.countInstances(new Search().addFilterEqual("clientGender", ClientGender.MALE));
        this.numberFemale = clientservice.countInstances(new Search().addFilterEqual("clientGender", ClientGender.FEMALE));
